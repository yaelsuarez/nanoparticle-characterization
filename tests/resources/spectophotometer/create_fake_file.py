...
def create_ref_line(start, stop, samples, scale):
    ...
    y = scale * (np.sin(2*x/(start-stop)+3.14) + np.random.normal(0, 0.001, samples) + 1)
    return x, y

def get_gaussian(x, scale=None, mu=None, sigma=None):
    scale = 5e4 + 1e5 * np.random.random() if scale is None else scale + 1e4 * np.random.random() - 5e3
    mu = random.randrange(500,800) if mu is None else mu + 10 * np.random.random() - 5
    sigma = random.randrange(1,50) if sigma is None else sigma + np.random.random()
